    ...
    class PrivateReflectionDynamicObject : DynamicObject
    {
        ...
        private static object InvokeMemberOnType(Type type, object target, string name, object[] args)
        {
            try
            {
                ...
                var methods = type.GetTypeInfo().GetMethods(BindingFlags.InvokeMethod 
                | bindingFlags).Where(m=>m.Name == name);
                
                if(methods.Count()==1)
                    return methods.First().Invoke(target, args);
                
                foreach(var method in methods)
                {
                    var ptypes = method.GetParameters().Select(p=>p.ParameterType).ToArray();
                    // Intersect is expensive but only approach i could see with netstandard
                    if((ptypes.Length == types.Length && ptypes.Intersect(types).Count() == ptypes.Length))
                    {
